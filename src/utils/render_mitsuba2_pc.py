...
import Imath # pip install openexr
import cv2
...
def standardize_bbox(pcl, points_per_object, shuffle=False):
    if shuffle:
...
def convert_exr_to_jpg(exr_file, jpg_file):
    ...
    image = imageio.imread(exr_file, format='exr')
    ...
    # imageio.imwrite(jpg_file, rgb_image, format='png')
