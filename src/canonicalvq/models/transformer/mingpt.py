...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size, block_size, n_layer=12, n_head=8, n_embd=256,
                 embd_pdrop=0., resid_pdrop=0., attn_pdrop=0., n_unmasked=0, cond_feature_dim=0):
        super().__init__()

        additional_block = 1 if cond_feature_dim > 0 else 0
        
        config = GPTConfig(vocab_size=vocab_size, block_size=block_size+additional_block,
                           embd_pdrop=embd_pdrop, resid_pdrop=resid_pdrop, attn_pdrop=attn_pdrop,
                           n_layer=n_layer, n_head=n_head, n_embd=n_embd,
                           n_unmasked=n_unmasked)
        # input embedding stem
        self.tok_emb = nn.ModuleList([nn.Embedding(config.vocab_size, config.n_embd) for i in range(block_size)])
        self.pos_emb = nn.Parameter(torch.zeros(1, config.block_size+additional_block, config.n_embd)) #  + cond_feature_dim
        ...
        self.block_size = config.block_size + additional_block
    ...
    def forward(self, idx, embeddings=None, targets=None, num_unique_groups=None):
        # forward the GPT model
        # token_embeddings = self.tok_emb(idx) # each index maps to a (learnable) vector

        token_embeddings = []
        for t in range(num_unique_groups):
            token_embeddings.append(self.tok_emb[t](idx[:,t])[:, None,:])
        token_embeddings = torch.concat(token_embeddings, dim=1)

        if embeddings is not None: # prepend explicit embeddings
            token_embeddings = torch.cat((embeddings.unsqueeze(1), token_embeddings), dim=1)
        ...
        if embeddings is not None:
            logits = logits[:, 1:] # cut off conditioning outputs
    ...
    def forward_with_past(self, idx, embeddings=None, targets=None, past=None, past_length=None):
        ...
        t = 0 if past_length is None else past_length
        token_embeddings = self.tok_emb[t](idx)    # each index maps to a (learnable) vector
        if embeddings is not None and t == 0:      # prepend explicit embeddings
...
@torch.no_grad()
def sample(model, x, steps, temperature=1.0, sample=False, top_k=None, embeddings=None):
    """
...
def sample_with_past(x, model, steps, temperature=1., sample_logits=True,
                     top_k=50, top_p=0.92, callback=None, embeddings=None): # 600, 0.92
    # x is conditioning
    sample = x
    cond_len = x.shape[1]
    past = None
    for n in range(steps):
        if callback is not None:
            callback(n)
        logits, _, present = model.forward_with_past(x, past=past, past_length=(n+cond_len-1), embeddings=embeddings)
